                        ...
                        double *beta)
{
    ...
    i = graph_fused_lasso_augmented_warm(n, y, w, ntrails, trails, breakpoints, lam, lambda2, alpha, inflate, maxsteps, converge, beta, z, u);
...
/* Augmented */
int graph_fused_lasso_augmented_warm (int n, double *y, double *w,
				        int ntrails, int *trails, int *breakpoints,
				        double lam, double lambda2, double alpha, double inflate,
				        int maxsteps, double converge,
				        double *beta, double *z, double *u)
{
    ...
    while(step < maxsteps && cur_converge > converge)
    {
        /* Update beta */
        if (w)
            update_beta_augmented(n, y, w, z, u, nzmap, zmap, alpha, beta, lambda2);
        else
            update_beta(n, y, z, u, nzmap, zmap, alpha, beta);
...
/* Augmented beta */
void update_beta_augmented(int n, double *y, double *w, double *z, double *u, int *nzmap, int *zmap, double alpha, double *beta, double lambda2)
{
    int i;
    int ridx;
    int zmap_idx;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        beta[i] = (2.0 * y[i] * w[i] + alpha * r) / (2.0 * w[i] + 2*lambda2 + alpha * nzmap[i]);
    }
}
